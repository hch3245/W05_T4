...
void FEditorViewportClient::Input()
{
    ...
    {
        if (UActorComponent* PickedComponent = GEngine->GetWorld()->GetSelectedComponent())
        {
            USceneComponent* PickedSceneComponent = Cast<USceneComponent>(PickedComponent);
            if (PickedSceneComponent == nullptr) 
            {
                PickedSceneComponent = PickedComponent->GetOwner()->GetRootComponent();
            }
            ...
            ViewTransform.SetLocation(
                ...
                PickedSceneComponent->GetWorldLocation() - (ViewTransform.GetForwardVector() * 10.0f)
            );
        }
    }
}
...
{
    float orthoDistance = 100.0f;

    switch (ViewportType)
    {
    case LVT_OrthoXY: // Top
        ViewTransformOrthographic.SetLocation(Pivot + FVector::UpVector* orthoDistance);
        ViewTransformOrthographic.SetRotation(FVector(0.0f, 90.0f, -90.0f));
        break;
    case LVT_OrthoXZ: // Front
        ViewTransformOrthographic.SetLocation(Pivot + FVector::ForwardVector* orthoDistance);
        ViewTransformOrthographic.SetRotation(FVector(0.0f, 0.0f, 180.0f));
        break;
    case LVT_OrthoYZ: // Left
        ViewTransformOrthographic.SetLocation(Pivot + FVector::RightVector* orthoDistance);
        ViewTransformOrthographic.SetRotation(FVector(0.0f, 0.0f, 270.0f));
        break;
    case LVT_Perspective:
        break;
    case LVT_OrthoNegativeXY: // Bottom
        ViewTransformOrthographic.SetLocation(Pivot + FVector::UpVector * -1.0f* orthoDistance);
        ViewTransformOrthographic.SetRotation(FVector(0.0f, -90.0f, 90.0f));
        break;
    case LVT_OrthoNegativeXZ: // Back
        ViewTransformOrthographic.SetLocation(Pivot + FVector::ForwardVector * -1.0f* orthoDistance);
        ViewTransformOrthographic.SetRotation(FVector(0.0f, 0.0f, 0.0f));
        break;
    case LVT_OrthoNegativeYZ: // Right
        ViewTransformOrthographic.SetLocation(Pivot + FVector::RightVector * -1.0f* orthoDistance);
